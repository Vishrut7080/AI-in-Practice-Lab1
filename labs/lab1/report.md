...
Provider: **Gemini 3.5 Flash Lite** (course `SMALL` tier), free tier capped at 15 req/min → all evals run with `--workers 1`; rate-limit gaps cleared by exact re-runs exploiting the response cache (a cached pass is ~instant and records `cost_usd` only for uncached calls). The numbers below come from the **test split, run once** on the shipped extractor. Since this report was first written I also tested one candidate prompt rewrite on the test split — it regressed the graded metrics and was **reverted**; that experiment, with its numbers, is the negative result in §6. No third test run was taken.
...
1. **Urgency boundary reading — 45 errors.** The 1/2 (answerable vs must-act) and 4/5 (tense: *threat* vs *statement* of ombudsman escalation) boundaries, plus 1↔3 confusion. Fix: a *decision procedure* in `urgency`'s description and the tense test for 4/5, plus 3-shot self-consistency at temperature 0.7 taking the median. **Caveat — I tried the decision-procedure half and it regressed test urgency (0.625 → 0.492); it was reverted, see §6.** What remains untried for real is the self-consistency lever. Worth ~0.625 → 0.78 if it lands, which alone moves record accuracy ≈ +0.15.
...
**Straw 1 — richer anchor descriptions.** Rewriting the `urgency`/`sentiment` descriptions (anchor-1/3/5 bullets, prior-failure test) did **not** lift record accuracy: C's dev record accuracy is exactly B's 0.4000, and on test `urgency` (0.625) is still the worst field. Why it failed: boundary labels are decided by *procedures* (answerability, tense), and richer prose nudges the model toward the nearest anchor's *tone* instead of the decision test — attention reads the examples, not the logic.

**Straw 2 — a decision-procedure description, tested once on test and reverted.** Taking my own §4 advice, I rewrote `urgency` as an ordered decision tree ("1 if answerable without opening the record …; a question about the customer's own data is 2, never 1; a wrong outcome is 3 even if calm; **THREAT** future tense is 4, **STATES** past/present is 5"), plus a fact-based `sentiment` rule ("sign-off/emoji/length are not tone"). On dev this looked excellent — record 0.4000 → **0.5167**, sentiment 0.7083 → 0.817 — and it fixed the two tickets that had failed the smoke test. But the single honest check on test regressed the graded metrics: **record 0.4333 → 0.3917, field 0.901 → 0.883, urgency 0.625 → 0.492** (sentiment did improve, 0.7083 → 0.7583). The failure mode moved rather than shrinking: the "own-data is 2, never 1" rule created a **19-item gold-1→2 over-escalation** cluster it had not had before. **The change was reverted and the test split was not used again.** The lesson, sharpened: what moved record accuracy in this lab was *structural* (moving deterministic fields into code, evidence-before-category), not *descriptive* — for these boundary fields, both a tone-based and a procedure-based description land within noise of each other, and the honest conclusion is that the cheap baseline is the best measured configuration. (Runner-up negative: the free-tier 429 back-off — the harness's max 4 retries at fixed delay exhaust before Gemini's ~56 s retryDelay, so straight re-runs, not retries, were the only thing that cleared quota.)
