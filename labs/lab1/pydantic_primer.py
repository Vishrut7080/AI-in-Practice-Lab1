...
class Ticket4(BaseModel):
    ...
    # Can also use:
    # policy_number: str | None = Field(default=None, pattern=r"^AUR-\\d{7}$")
    policy_number: Optional[str] = Field(default=None,pattern=r"^AUR-\d{7}$")
...
class Ticket5(BaseModel):
    ...
    urgency:int=Field(ge=1,le=5,description="Since model can't understand how urgent the task is, " \
    "we are using numbers to show the urgency. Urgency 1 to 5 tell the urgency. 1-something general like cutomer is waiting" \
    "3-some task that is supposed to be done eod")
...
class TicketRecord(BaseModel):
    ...
    evidence:str=Field(max_length=200, description='Evidence for the Ticket')
    category:CATEGORIES=Field(description='Categories used to prioritize the ticker')
    urgency:int=Field(le=5,ge=1,description='Urgency of the ticket, 1 (low) to 5 (critical)')
    policy_number:Optional[str]=Field(default=None,
                                      pattern=r'^AUR-\d{7}$',
                                      description='Policy number in AUR-XXXXXXX format')
    @field_validator('policy_number',mode='before')
    @classmethod
    def _clean(cls,v):
        if v is None:
