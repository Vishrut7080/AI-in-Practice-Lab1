...
def main() -> None:
    ...
    check("ticket data present", lambda: (
        f"{sum(1 for _ in open(ROOT/'data/eval/extraction_dev.jsonl', encoding='utf-8'))} dev cases"))
    check("golden RAG set present", lambda: (
        f"{sum(1 for _ in open(ROOT/'data/eval/rag_golden.jsonl', encoding='utf-8'))} questions"))
